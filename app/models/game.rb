class Game < ApplicationRecord
    ...
    def convert_image
        unless title_picture.blob.content_type == 'image/png'
           # ToDo: Refactor!
            temp = Tempfile.open('temp.jpg', 'wb') do |file| 
            ...
            title_picture.attach(io: Tempfile.open("temp.png"), filename: "#{name}.png", content_type: "image/png")

            index_image = Magick::Image.read('temp.png').first
            index_image = index_image.resize_to_fit('400')
            ...
            index_picture.attach(io: Tempfile.open("temp.png"), filename: "index_#{name}.png", content_type: "image/png")
            # maybe delete temp files
            Tempfile.unlink('temp.png')
            Tempfile.unlink('temp.jpg')
            return
        end
    end
end
